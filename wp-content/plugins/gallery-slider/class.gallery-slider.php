...
class gallerySlider { 
    ...
    const PLUGIN_NAME = 'Gallery Slider'; 
    const PLUGIN_PREFIX = 'gallerySlider_';
    ...
    public function __construct($file,$ver) {
        ...
        if (is_admin()) { 
            ...
            add_shortcode("gallery-slider", array(__CLASS__,'cb_shortcode' ));
        }
    }
    
    public static function cb_admin_init () {
        wp_register_script('gallery-admin-js', self::$plugin_dir_url.'assets/js/jquery.gallery-admin.js',array('jquery'),self::$version,true);
        wp_register_style('gallery-admin-css', self::$plugin_dir_url.'assets/css/gallery-admin.css',null,self::$version,'all'); 
    }
                ...
                if(isset($_POST['sliderID']) || isset($_POST['slider'])) { ?> 
                    ...
                    <html> 
                        <body style="padding:0px;margin:0px;overflow:hidden">
                            ...
                            <script type="text/javascript" src="<?php echo self::$plugin_dir_url ?>assets/js/jquery.gallery.js"></script> 
                            <link href="<?php echo self::$plugin_dir_url?>assets/css/gallery.css" rel="stylesheet" type="text/css" />
                            <?php
                                if(isset($_POST['sliderID']) && isset($_POST['slider'])) {
    ...
    public static function cb_admin_print_styles () {
        ...
        wp_enqueue_style('gallery-admin-css'); 
    }

    public static function cb_admin_print_scripts () {

        wp_enqueue_script('gallery-admin-js');   
    }

    public static function cb_wp_enqueue_scripts () { 
        wp_enqueue_script('gallery-front-js', self::$plugin_dir_url.'assets/js/jquery.gallery.js', array('jquery'), self::$version );
        wp_enqueue_style ('gallery-front-css',self::$plugin_dir_url.'assets/css/gallery.css',null, self::$version );
    }
    ...
    public static function cb_admin_enqueue_scripts () {
        ...
        wp_localize_script('gallery-admin-js', 'JS_DATA', array(
